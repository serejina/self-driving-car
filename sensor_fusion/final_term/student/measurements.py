...
class Sensor:
    ...
    def in_fov(self, x):
        ...
        pos_veh = np.ones((4, 1)) # homogeneous coordinate
        pos_veh[0:3] = x[0:3]
        pos_sens = self.veh_to_sens * pos_veh

        # x lies in the sensor's field of view?
        if pos_sens[0] > 0:
    ...
    def get_hx(self, x):    
        ...
        if self.name == 'lidar':
            ...
            pos_veh = np.ones((4, 1)) # homogeneous coordinates
            pos_veh[0:3] = x[0:3]
            pos_camera = self.veh_to_sens * pos_veh # transform from vehicle to camera coordinates
            # make sure to not divide by zero, raise an error if needed
            if pos_camera[0] != 0:
                # project from camera to image coordinates
                hx = np.zeros((2, 1))
                hx[0,0] = self.c_i - (self.f_i* pos_camera[1]/pos_camera[0])
                hx[1, 0] = self.c_j - (self.f_j * pos_camera[2] / pos_camera[0])
                return hx
            else:
                raise ZeroDivisionError("Invalid coordinate system")
