...
// returns index of the closest point in planner to current state of vehicle
int get_closest_point_index(double veh_position_x, double veh_position_y, vector<double> planner_points_x, vector<double> planner_points_y)
{ 
  size_t index = 0;
  double dist_min;        
  for(auto i=0; i < planner_points_x.size(); ++i){
    //calc distance between planner points and vehicle position
    double distance = pow((pow((veh_position_x - planner_points_x[i]), 2) + 
                             pow((veh_position_y - planner_points_y[i]), 2)),
                          0.5);
  ...
  h.onMessage([&pid_steer, &pid_throttle, &new_delta_time, &timer, &prev_timer, &i, &prev_timer](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode)
  {
        ...
        if (s != "") {
          ...
          * TODO (step 3): compute the steer error (error_steer) from the position and the desired trajectory
          The error is the angle difference between the actual steer and the desired steer 
          to reach the planned position.
          **/
          error_steer = yaw - angle_between_points(x_position, y_position, x_points[closest_idx], y_points[closest_idx]);
