...
// Implementation of the PCL NDT function. The function returns the correct transformation matrix. If NDT hasn't converged, it returns the identity matrix
Eigen::Matrix4d NDT(pcl::NormalDistributionsTransform<pcl::PointXYZ, pcl::PointXYZ> ndt, PointCloudT::Ptr source, Pose startingPose, int iterations){
  	...
  	const auto init_guess = transform3D(startingPose.rotation.yaw, startingPose.rotation.pitch, startingPose.rotation.roll, startingPose.position.x, startingPose.position.y, startingPose.position.z).cast<float>();
	...
	if (ndt.hasConverged())
	{
  	...
  	std::cout << "NDP has not converged" << std::endl;
	return transformation_matrix;
}

// Implementation of the PCL ICP function and return the correct transformation matrix. If ICP hasn't converged, it returns the identity matrix
Eigen::Matrix4d ICP(PointCloudT::Ptr target, PointCloudT::Ptr source, Pose startingPose, int iterations){

  	Eigen::Matrix4d transformation_matrix = Eigen::Matrix4d::Identity ();

  	auto initTransform = transform3D(startingPose.rotation.yaw, startingPose.rotation.pitch, startingPose.rotation.roll, startingPose.position.x, startingPose.position.y, startingPose.position.z);
	...
	while (!viewer->wasStopped())
  	{
		...
		if(!new_scan){
			...
			Eigen::Matrix4d pose_transform = NDT(ndt, cloudFiltered, pose, 100);
			//Eigen::Matrix4d pose_transform = ICP(mapCloud, cloudFiltered, pose, 100);
