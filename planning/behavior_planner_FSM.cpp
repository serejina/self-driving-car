...
double BehaviorPlannerFSM::get_look_ahead_distance(const State& ego_state) {
  ...
  auto look_ahead_distance = velocity_mag * _lookahead_time + 0.5 * accel_mag * std::pow(_lookahead_time, 2);
...
State BehaviorPlannerFSM::state_transition(const State& ego_state, State goal,
  ...
  if (_active_maneuver == FOLLOW_LANE) {
    ...
    if (is_goal_in_junction) {
      ...
      goal.velocity.x = 0.0;  // <- Fixed
      goal.velocity.y = 0.0;  // <- Fixed
      ...
      goal.velocity.x = _speed_limit * std::cos(goal.rotation.yaw);  // <- Fixed
      goal.velocity.y = _speed_limit * std::sin(goal.rotation.yaw);  // <- Fixed
      goal.velocity.z = 0;
    }
    ...
    // the stop line. goal = ...;
    goal = _goal;  // Keep previous goal. Stay where you are. // <- Fixed
